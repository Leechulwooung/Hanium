...
void My_waypoint::goal_select(){
  ...
  if(web_first_toggle == true){
    ...
    // 첫 번째 시나리오(첫 번째, 두 번쨰 사용자)가 끝나고 세 번째 사용자가 사용을 할 때 
    // short_distance 함수에서 website_first_answer == 9(receive)인 경우로 들어갈 때 움직이지 않게 하기 위해서. 
    // -> 이걸 하지 않으면 그 전 사용자 2의 목적지가 남아 있어서 사용자 2의 목적지로 넘어간다.
    // second_pose_stamped를 초기화하기 위해서.
    // 이 이유로 인해 우리는 사용자가 한 명이라면 사용자 1만 사용해야한다.
...
void My_waypoint::call_goal_select(){
  ...
  if(call_toggle == true && call_check == 0){
    ...
    // take one이 recived를 하지 않고 give two가 거점 선택을 하지 않았을 경우
    if(call_block == 0 && second_check == 0){

      goal_pub.publish(call_pose_stamped);

    }
    call_check = 1;      // 호출을 한 번 한 경우를 구분하기 위해 call_check 변수를 1로 설정
    call_toggle = false;
...
///////////void 초기 등록 거점 과 경유지에서 등록 거점 비교 함수(), website_second_answer == 9 : take_one / website_second_answer == 10 : take_two
void My_waypoint::short_distance(){

  if(website_second_answer == 5 || website_second_answer == 6 || website_second_answer == 7 || website_second_answer == 8 || website_second_answer == 9 || website_second_answer == 9 || website_second_answer == 10){

    ROS_INFO("click : %d", website_second_answer);
    ...
    if(sqrt(abs(pow((call_pose_stamped.pose.position.x - first_pose_stamped.pose.position.x), 2) + pow((call_pose_stamped.pose.position.y - first_pose_stamped.pose.position.y), 2))) 
      ...
      ROS_INFO("click_first : %d", website_second_answer);
    ...
    // 호출한 위치에서 사용자 1의 목적지가 사용자 2의 목적지 보다 거리가 먼 경우
      ...
      ROS_INFO("click_second : %d", website_second_answer);
    ...
    // 호출한 위치에서 사용자 1의 목적지가 사용자 2의 목적지의 거리가 같은 경우, 먼저 배송을 보낸 사용자 1의 목적지에 먼저 가라.
      ...
      ROS_INFO("click_third : %d", website_second_answer);
    ...
    // website_second_answer/ website_first_answer 데이터값 초기화
    website_first_answer = 0;
    website_second_answer = 0;
...
void My_waypoint::priority(){
  ...
  if(website_second_answer == 5 || website_second_answer == 6 || website_second_answer == 7 || website_second_answer == 8 || website_second_answer == 9 || website_second_answer == 10){
      
      ROS_INFO("click : %d", website_second_answer);
      ROS_INFO("call_block : %d", call_block);

      //second user에서 거점을 지정하면 first user의 거점으로 이동
      if(website_second_answer == 5 || website_second_answer == 6 || website_second_answer == 7 || website_second_answer == 8 ){
        // 사용자 1의 목적지로 가라
        goal_pub.publish(first_pose_stamped);       
      }
      // take one이 물건을 수령한 후 'received' 버튼을 누르면 다음 목적지로 간다. give two의 거점이 선택되는 것을 확인(second_check == 1) & 호출을 한 번만 하게 막는다.
      else if(call_block == 0 && second_check == 1 && website_second_answer == 9){//recieved가 들어왔을때
        goal_pub.publish(second_pose_stamped);   // 사용자 2의 목적지로 가라
        call_block = 1;                          // take one 에서 received를 눌렀음을 확인
        call_check = 0;                          // 호출을 할 수 있게끔 만들어준다.

        call_pose_stamped = second_pose_stamped; // take two 에서 received를 눌렀을 때 로봇이 그 거점을 유지하도록 설정(초기화)

        std::cout << "call check 1? " << call_block << std::endl;
        std::cout << "  " << std::endl;

      }
      // take two이 물건을 수령한 후 'received' 버튼을 누르면 다음 호출 거점으로 간다. or 호출을 하지 않았을 시 그대로 있음
      else if(call_block == 1 && website_second_answer == 10){
